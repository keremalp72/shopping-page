const products = [
  ...
  function handleSubmit(e) {
    e.preventDefault();
   filteredText = filterForm.filteredText.value;
    render();
  }
  ...
  function render() {
    ...
    for (const product of products) {
      if (product.title.toLocaleLowerCase("en").includes(filteredText) !== true) {
        continue;
      }
    ...
    <strong>$${product.price}</strong> | <strong>${product.stock}</strong>
    </div>
    <ul class="product-tags">${getProductTagsHtml(product.tags)}</ul>
    </li>`;
      //pozitif bir şeyler kontrol etme istediğimizde if sayıları çok artar . o yüzden negatifleri kontrol edersek ve akışı değiştirirsek yazdığımız kodlar daha okunaklı olur
    }
  }
  render();
  //getProductTagsHtml fonskiyonunu çağırıyoruz ama ne işe yarıyor ?
  //yeni bir fonksiyon ile elimdeki etiket(tags) leri gönderiyorum ki bir döngü oluşturmam lazım. çükü 3 etiket olabilir
  //hiç etiket olabilir bilmediğim için döngüye yazmam gerekir
  //burada for döngüsü kullnamadığımız için bir fonksiyon içinde for döngüsü ile tagsleri döndüm
  //bunu farklı bir yolla yapmak istesem map ile yapmam lazım.
  //fakat şuan daha kullanmadığımız için onu es geçiyoruz.onun yerine yardımcı fonksiyon yazdık.
  
  // const students = [
  // "mücahit","elif","şura","berk","kerem","irem","bora","ali rıza","muhammed","zennur"
  // ];
  
  // let aranan = "bora";
  
  // for (const x of students) {
  //   console.log(`şuan ${x} ile ${aranan} kontrol ediyorum`);
  //   if(x === aranan){
  //     console.log("buldum");
  //     break;//aranan öğrenciyi bulduktan sonra döngünün devam eetmesine gerek yok.döngünün bruada kırılması yada durması gerekiyor
  //   }
  // }
  // for (const x of students) {
  //   if(x !== aranan){
